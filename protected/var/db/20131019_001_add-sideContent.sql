ALTER TABLE  `blogRecords` ADD  `sideContent` LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL AFTER  `content` ;

CREATE OR REPLACE VIEW `blogRecords_view` AS select `blogRecords`.`idRecord` AS `idRecord`,`blogRecords`.`idCat` AS `idCat`,`blogRecords`.`idTree` AS `idTree`,`blogRecords`.`uidRec` AS `uidRec`,`blogRecords`.`title` AS `title`
